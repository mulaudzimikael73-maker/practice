...
function dailyJobs(){
 const JOB_ROTATION_V2="154-to-152-v2";
 let state=read(KEYS.jobs,{date:"",selected:[],completed:{},version:""});
 if(state.date!==today() || state.version!==JOB_ROTATION_V2){
   const ranked=[...JOBS].sort((a,b)=>(hash(today()+a.id)%100000)-(hash(today()+b.id)%100000));
   // Force several of the newly added jobs into today's board once, then return
   // to the normal full-pool shuffle on the next day.
   const todayNew=["dare_143_new","dare_147_new","dare_151_new"];
   const forced=todayNew.map(id=>JOBS.find(x=>x.id===id)).filter(Boolean);
   const picked=[...forced];
   for(const j of ranked){if(picked.length>=5)break;if(!picked.some(x=>x.id===j.id))picked.push(j)}
   state={date:today(),selected:picked.slice(0,5).map(x=>x.id),completed:{},version:JOB_ROTATION_V2};
...
// One-time repair for the Reverse Token already earned by Lizzy.
// The Cloudflare inventory is checked first, so an existing token is never duplicated.
const NICE_PHOTO_TOKEN="Reverse Token — Mikael Gets One Nice Photo";
const NICE_PHOTO_REPAIR="lizzyMikaelNicePhotoRepairV1";
async function repairNicePhotoToken(){
  if(localStorage.getItem(NICE_PHOTO_REPAIR)==="done")return;
  try{
    const state=await cloudState();
    const current=Number(state?.inventory?.[NICE_PHOTO_TOKEN]||0);
    if(current>0){localStorage.setItem(NICE_PHOTO_REPAIR,"done");return;}
    const d=DEFS[NICE_PHOTO_TOKEN];
    if(!d)return;
    const awarded=await post({type:"mikael_reverse_token_award",name:NICE_PHOTO_TOKEN,emoji:d.emoji,desc:d.desc,source:"Daily Reward Repair"});
    if(awarded?.success){localStorage.setItem(NICE_PHOTO_REPAIR,"done");}
  }catch(e){console.warn("Nice photo token repair pending",e)}
}
...
setTimeout(async()=>{try{await migrate()}catch(e){}try{await repairNicePhotoToken()}catch(e){}await renderPublic();await checkPending()},900);
