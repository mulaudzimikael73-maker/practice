...
function dailyJobs(){
 let state=read(KEYS.jobs,{date:"",selected:[],completed:{},rotationVersion:0});
 const ROTATION_VERSION=2;
 const TODAY_FEATURED_JOBS={
   "2026-09-11":["job_143_press_conference","job_147_mickys_got_talent","job_151_natures_most_dangerous_creature"]
 };
 if(state.date!==today() || state.rotationVersion!==ROTATION_VERSION){
   const featured=(TODAY_FEATURED_JOBS[today()]||[]).filter(id=>JOBS.some(j=>j.id===id));
   const featuredSet=new Set(featured);
   const ranked=[...JOBS]
     .filter(j=>!featuredSet.has(j.id))
     .sort((a,b)=>(hash(today()+a.id)%100000)-(hash(today()+b.id)%100000));
   const selected=[...featured,...ranked.slice(0,5-featured.length).map(x=>x.id)];
   state={date:today(),selected,completed:{},rotationVersion:ROTATION_VERSION};
