...
export const Route = createFileRoute("/personality-lab")({
  head: () => ({
    meta: [
...
const PERSONAS: Persona[] = [
  {
    ...
    screenName: "attitude-book pro",
    compliments: [
      "Dangerously pretty. Handle with caution. 😏",
      "She's a baddie and the system has receipts. 🔥",
      "Face card never declines. Ever.",
      "Certified menace, unfortunately gorgeous. 💅",
      "Walked in and ruined everyone's average.",
      "Sassy, iconic, slightly illegal levels of cute.",
    ],
  },
  {
    name: "Lizzy",
    ...
    screenName: "lizzy-book air",
    compliments: [
      "You're the softest part of my whole system. 💗",
      "Diagnostics confirm: still the prettiest girl online.",
      "Your smile boosted performance by 100%. 🌸",
      "Sweetest user ever detected. No further scans needed.",
      "You make everything feel like a good day. ☀️",
      "Flowers wish they had your energy. 🌷",
    ],
  },
  {
    ...
    screenName: "agent-book classified",
    compliments: [
      "INTEL REPORT: dangerously pretty. Threat level: maximum. 🕵️",
      "Target is beautiful AND armed with common sense.",
      "Classified file: 'she's a baddie'. Access restricted.",
      "Charm confirmed as a lethal weapon. Noted.",
      "Mission success rate rises when you smile. Suspicious.",
      "No agency on earth is prepared for you.",
    ],
  },
];
...
function MacBook({ persona }: { persona: Persona }) {
  ...
  useEffect(() => {
    const id = window.setInterval(() => {
      setIndex((i) => (i + 1) % persona.compliments.length);
    }, 4200);
    return () => window.clearInterval(id);
  }, [persona.name, persona.compliments.length]);

  const compliment = persona.compliments[index] ?? persona.compliments[0]!;

  return (
    <section className="mt-9">
      <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-accent">
        {persona.name}'s MacBook
      </span>

      <div className="mx-auto mt-4 w-full max-w-xl">
        <div
          className="rounded-2xl border border-border p-2 shadow-[var(--glow)] sm:p-3"
          style={{ background: "var(--panel)" }}
        >
          <div className="relative overflow-hidden rounded-xl border border-border bg-background">
            <img
              src={persona.photo}
              alt={`${persona.name} wallpaper`}
              className="h-56 w-full object-cover opacity-40 sm:h-64"
            />
            <div className="absolute inset-0 flex flex-col justify-between p-4 sm:p-5">
              <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
                <span>{persona.screenName}</span>
                <span aria-hidden>{persona.emoji} 100%</span>
              </div>
              ...
              <div className="rounded-xl border border-border bg-card/80 p-4 backdrop-blur-sm">
                ...
                <p key={compliment} className="mt-2 animate-in fade-in text-base font-semibold leading-snug sm:text-lg">
                  {compliment}
                </p>
              </div>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setIndex((i) => (i + 1) % persona.compliments.length)}
                  className="rounded-full bg-primary px-4 py-2 text-xs font-bold text-primary-foreground transition hover:opacity-90"
                >
                  Another one {persona.emoji}
                </button>
              </div>
            </div>
          </div>
        </div>
        <div className="mx-auto h-2 w-3/4 rounded-b-2xl border border-t-0 border-border bg-secondary" />
        <div className="mx-auto h-1 w-1/4 rounded-b-full bg-border" />
      </div>
    </section>
  );
}
